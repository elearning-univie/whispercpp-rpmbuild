...
cd %{_builddir}/target
cublasCXX=$(find /usr/local -name "nvcc") CUDACXX=$(find /usr/local -name "nvcc") cmake -DWHISPER_STANDALONE:BOOL=OFF -DBUILD_SHARED_LIBS:BOOL=ON -DWHISPER_OPENBLAS:BOOL=ON -DWHISPER_CUBLAS:BOOL=ON -DWHISPER_STATIC:BOOL=OFF ..
make clean
make -j$(nproc)
